...
class HomeController extends Controller
{
    ...
    public function searchTiquet(Request $request){
        if($request->get('data'))
        {
            $ticket = Ticket::where('ticket_id',$request->get('data'))->first();
            ...
            if(isset($ticket)){
                ...
                $output .= '<li><a href="'
                        .url('/guest/ticket/'.$ticket->ticket_id).
                        
                        '" class=""  style="color:black;">' . $ticket->subject . '<span class="badge bg-success-transparent text-success float-end fs-11">'.$ticket->status.'</span></a>
                        </li>';
